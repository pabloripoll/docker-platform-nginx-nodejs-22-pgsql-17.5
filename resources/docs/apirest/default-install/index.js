...
app.get('/', async (req, res) => {
  let dbstatus = true;
  let dbstatus_message = '<span>POSTGRES database successfully connected</span>';
  ...
  try {
    ...
    dbstatus = false;
    dbstatus_message = `<b style="color:red">POSTGRES connection error:</b> ${e.message}`;
  }
...
<body>
    ...
    <header>
        <h1>PLATFORM <span>API REST</span> - Node ${nodeVersion} (${platform})</h1>
    </header>
    <p>Container is running successfully and serving plain <code>index.js</code> script with <span>HTML5</span> on <code>./public</code> folder.</p>
